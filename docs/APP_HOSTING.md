...
- **SSR widget checks** on **`/u/[username]`** (and tenant-root **`/`**) use **`NEXT_PUBLIC_CLOUD_FUNCTIONS_APP_ORIGIN`** (or **`SERVER_WIDGET_FETCH_ORIGIN`**) as the fetch base in production, not the browser hostname, so the App Hosting server does not call its own public tenant URL (which often produced **5xx** while browser **`/api/widgets/...`** on the same host still worked). **`AuthProvider`** skips **`/api/client-auth-config`** on **`/u/*`** and on **`/`** when the host is listed in the tenant map.

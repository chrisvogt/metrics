...
const fetchPhotos = async () => {
  ...
  try {
    ...
    const res = body as { photos?: { photo?: unknown[]; total?: number; page?: number; pages?: number } }
    if (!res?.photos?.photo) {
      throw new Error('Invalid response from Flickr API')
    }

    return {
      photos: res.photos.photo.map((photo: Record<string, unknown>) => ({
        id: photo.id,
        title: photo.title,
        description: (photo.description as Record<string, unknown>)?._content || '',

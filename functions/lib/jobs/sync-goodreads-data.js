...
const processUpdatesWithMedia = async (updates = [], books = []) => {
    ...
    if (updatesNeedingBooks.length > 0) {
        ...
        const fetchedBookResults = await pMap(Array.from(uniqueBooksToFetch.values()), async ({ update, isbn, book, updates }, index) => {
        ...
        const fetchedBooksWithMetadata = fetchedBookResults
            .filter(({ googleBookResult }) => Boolean(googleBookResult) && Boolean(googleBookResult.book))
...
const fetchAllGoodreadsPromises = async () => {
    try {
        ...
        const u = (user ?? {});
        const r = (recentlyRead ?? {});
        const processedUpdates = u.updates == null ? null : await processUpdatesWithMedia(u.updates, r.books ?? []);
        return {
            collections: {
...
const syncGoodreadsData = async () => {
    const { collections = {}, error, profile = {}, responses = {} } = await fetchAllGoodreadsPromises();
    if (error) {
...
//# sourceMappingURL=sync-goodreads-data.js.map
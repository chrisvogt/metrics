...
const transformSteamGame = (game) => {
    const { appid: id, img_icon_url: iconHash, name: displayName, playtime_2weeks: playTime2Weeks, playtime_forever: playTimeForever, } = game;
    return {
        displayName,
        id,
        images: {
...
const syncSteamData = async () => {
    ...
    const ownedGamesObj = ownedGames;
    const playerSummaryObj = playerSummary;
    const { game_count: ownedGameCount = 0 } = ownedGamesObj;
    const { avatarfull: avatarURL, profileurl: profileURL, personaname: displayName, } = playerSummaryObj;
    const widgetContent = {
        collections: {
...
//# sourceMappingURL=sync-steam-data.js.map
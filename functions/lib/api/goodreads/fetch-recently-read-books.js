...
import { CLOUD_STORAGE_IMAGES_BUCKET, IMAGE_CDN_BASE_URL } from '../../lib/constants.js';
const toBookMediaDestinationPath = id => `books/${id}-thumbnail.jpg`;
const transformBookData = (book) => {
    const { book: { id, volumeInfo: { authors, categories = [], description, imageLinks: { smallThumbnail = '', thumbnail = '' } = {}, infoLink = '', pageCount, previewLink, subtitle, title, } = {}, } = {}, rating, goodreadsDescription, isbn, } = book;
...
export default async () => {
    ...
    const { body } = await got(`https://www.goodreads.com/review/list/${userID}.xml?key=${key}&v=2&shelf=read&sort=date_read&per_page=100`);
    ...
    const bookResults = await pMap(bookReviews, async (book, index) => {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (foundBook) {
                        ...
                        logger.info(`Found book by ${bookObj.authorName ? 'title/author' : 'title'} for recently read: ${bookObj.title}`);
                        break;
                    }
                }
                catch (error) {
                    ...
                    if (isQuotaExceeded) {
                        logger.error('Daily quota exceeded for Google Books API. Title/author fallback skipped.', { message: errorBody?.error?.message });
                        break;
                    }
                    if ((statusCode === 429 || statusCode === 503) && attempt < maxRetries) {
                        const waitTime = Math.pow(2, attempt) * 1000;
                        logger.warn(`Rate limited (${statusCode}) for title search "${bookObj.title}", waiting ${waitTime}ms before retry ${attempt}/${maxRetries}`);
                        await new Promise(resolve => setTimeout(resolve, waitTime));
                        continue;
                    }
                    logger.error(`Error fetching book by ${bookObj.authorName ? 'title/author' : 'title'} for recently read "${bookObj.title}" after retries:`, error);
                    break;
                }
            }
        }
        if (!result || !result.book) {
            logger.warn(`Book not found by ISBN or title/author fallback: ${bookObj.title ? `"${bookObj.title}"` : `ISBN ${bookObj.isbn}`}`);
        }
        return {
...
//# sourceMappingURL=fetch-recently-read-books.js.map
...
const getProfileFromResponse = (result) => {
    ...
    const { name, user_name: username, link, image_url: imageURL, small_image_url: smallImageURL, website, joined, interests, favorite_books: favoriteBooks, friends_count: { _: friendsCount = '' } = {}, } = rawProfile;
    return {
        name,
        username,
        link,
...
const getUpdatesFromResponse = (result) => {
    ...
    const validateUpdate = (update) => update && (update.type === 'userstatus' || update.type === 'review');

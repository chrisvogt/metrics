...
const fetchReleasesBatch = async (releases, options = {}) => {
    ...
    if (releasesWithoutResourceUrl.length > 0) {
        const r = (x) => x.release;
        const bi = (x) => x.basic_information;
    ...
    const results = await pMap(validFetchTasks, async ({ release, resourceUrl, releaseId }, index) => {
        try {
...
//# sourceMappingURL=fetch-releases-batch.js.map
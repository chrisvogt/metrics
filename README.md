...
#### TypeScript
The `functions/` package is TypeScript. Source lives in `functions/*.ts` and `functions/**/*.ts`; `pnpm run build` (from repo root or from `functions/`) compiles with `tsc` and outputs to `functions/lib/`. Firebase deploys from `lib/`. Run `pnpm run build` before `pnpm run deploy:functions` if you’ve changed function code. Tests (`pnpm run test`) and lint (`pnpm run lint`) run against the TypeScript source.
